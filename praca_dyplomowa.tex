...
Ilustrację rozkładu wieku osób biorących udział w badaniu przestawia rysunek \ref{rys:wiek}.
...
\caption{Wiek osób ankietowanych, Badania własne 2021/2022r.}
\label{rys:wiek}
\end{figure}
...
Zakres danych & 22 - 62\\
\hline
Odchylenie standardowe & 10.6754\\
\hline
...
\caption{Płeć osób ankietowanych, Badania własne 2021/2022r.}
\label{rys:plec}
\end{figure}
...
\begin{figure}
\includegraphics[width=8cm]{char_gr_bad/wyksztalc00}
\caption{Wykształcenie osób ankietowanych, Badania własne 2021/2022r.}
\label{rys:wykszt}
\end{figure}
...
Analizy wykonano w środowisku statystycznym R, wer. 4.2.0 \cite{rproject} oraz programie GNU PSPP, wer.: 1.4.1 \cite{gnupspp}


Metody statystyczne, którymi się posłużono, to:
\begin{itemize}
     \item Test Shapiro – Wilk, jest testem służącym do sprawdzenia założenia o normalności rozkładu zmiennej.
     \item Test Shapiro – Francia,  jest testem służącym do sprawdzania normalności rozkładu zmiennej w oparciu o duże >50 próbki danych. Wprowadzony przez S.S.Shapiro oraz R. S Francia w 1972 r. jako uproszczenie testu Shapiro - Wilk
    
     \item Test Pearsona $\chi^2$, test służący do oceny zależności pomiędzy rozkładem częstości odpowiedzi w zakresie jednej zmiennej, w odniesieniu do drugiej zmiennej.
...
Z analiz statystycznych wynika:
\begin{itemize}

    \item ilość godzin pracy nie ma istotnego wpływu na przynoszenie do domu pracy w postaci myśli i emocji $\chi^2 (2,115) = 0,224, p=0,89$
    
    \item doswiadcza traumatyzacji wtórnej    
    
    \item potrafi niwelować
    
    \item nie potrafi stosować technik psychologicznych
    
    \item w celu rozładowania stres sięga po uzywki
    
    
    \item istnieje korelacja pomiędzy ilością ponadnormatywnych godzin pracy a nasileniem konfliktów na linii praca-rodzina  $\chi^2 (2,115) = 6.4194, p=0,04037$
