...
\begin{itemize}
     ...
     \item Współczynnik korelacji liniowej Pearsona $R$, określający poziom zależności liniowej między zmiennymi losowymi.
     
     \Item Test trendu Mantel-Haenshel $\chi^2$ (Linear-by-Linear association), określający 
...
Test statystyczny wykazał istotną statystycznie różnice w kożystaniu ze zwolnień lekarskich gdy jest ku temu powód od ilości etyatów na któych pracuje pielęgniarz/arka. 

 zależności od wykształcenia (wartość statystyki testowej Kruskala-Wallisa 9,04; p=0,029). Testy post hoc pokazały, że globalne poczucie koherencji jest istotnie mniejsze u osób, które ukończyły średnią szkołę medyczną w porównaniu do absolwentów magisterskich studiów pielęgniarskich (p=0,026)

ile etatow koreluje negatywnie z Q24 (uczestniczy w zyciu rodzinnym)
czyli wiecej etatow mniej zycia rodzinnego
...
Odpowiedzi na to pytanie przedstawia tabela \ref{tab:Q31}.

% pytanie: 31 - 30. Czy  zdarza się, że pracuje Pan/i, mimo wystąpienia własnej choroby, choroby członka rodziny?
\begin{table}[h]
\caption{Pracuje, mimo wystąpienia chorobyb, własnej lub członka rodziny}
\centering
\begin{tabular}{ | c | c | c |}
\hline
...
% Q3 vs. Q13
Test statystyczny Pearsona $\chi^2$ wykazał istotne statystycznie różnice w sięganiu po używki w celu radzenia sobie ze stresem związanym z pracą zawodową w zależności od ilości godzin pracy miesięcznie. ($\chi^2 (2, 115) = 6,1152, p=0,047$) 
